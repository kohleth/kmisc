...
#' }
#' @export
cvAll = function(x, data, k, ...) UseMethod("cvAll")

#' @export
#' @rdname cvAll
cvAll.default <- function(x, data, k, lossfn, ind, finalfit, ...) {
    ## Train the models ---------------------
...
#' @export
#' @rdname cvAll
cvAll.lm = function(x, data, k = 5, lossfn = .calcMSE, ind = NULL, finalfit = TRUE, 
    ...) {
    ## Splliting out a validation set
    if (is.null(ind)) 
        ind = createSCV(form = formula(x), data = data, k = k)
    cvAll.default(x = x, data = data, k = k, ind = ind, lossfn = lossfn, 
        finalfit = finalfit, ...)
...
#' @export
#' @rdname cvAll
cvAll.lme = function(x, data, k = 5, lossfn = .calcMSE, ind = NULL, finalfit = TRUE, 
    ...) {
    ## Splliting out a validation set
    if (is.null(ind)) {
...
#' @export
#' @rdname cvAll
cvAll.merMod = function(x, data, k = 5, lossfn = .calcMSE, ind = NULL, 
    finalfit = TRUE, ...) {
    ## Splliting out a validation set
    if (is.null(ind)) 
